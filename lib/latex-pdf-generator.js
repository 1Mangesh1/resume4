...
class LatexPDFGenerator {
  ...
  parseLatexManually(latexCode) {
    ...
    const namePatterns = [
      /\\textbf\{([^}]+)\}(?=.*(?:engineer|developer|analyst|manager|specialist|consultant|architect))/i,
      /\\Large\\textbf\{([^}]+)\}/,
      /\\huge\{([^}]+)\}/,
      /\\LARGE\{([^}]+)\}/,
      /\\name\{([^}]+)\}/,
      /\\textbf\{\\LARGE\s+([^}]+)\}/,
      /^\\textbf\{([^}]+)\}/m, // Fallback: any bold text at start of line
    ];
    ...
    if (phoneMatch) data.contact.phone = phoneMatch[1].trim();

    const linkedinMatch = latexCode.match(/linkedin\.com\/in\/([^}\s\\]+)/);
    if (linkedinMatch)
      data.contact.linkedin = `linkedin.com/in/${linkedinMatch[1]}`;

    const githubMatch = latexCode.match(/github\.com\/([^}\s\\]+)/);
    if (githubMatch) data.contact.github = `github.com/${githubMatch[1]}`;

    // Enhanced section extraction with more flexible patterns
    const sectionPatterns = [
      {
        name: "Professional Summary",
        patterns: [
          /\\begin\{rSection\}\{(?:PROFESSIONAL SUMMARY|SUMMARY|PROFILE)\}(.*?)\\end\{rSection\}/s,
          /\\section\*?\{(?:Professional Summary|Summary|Profile)\}(.*?)(?=\\section|\s*$)/s,
        ],
      },
      {
        name: "Experience",
        patterns: [
          /\\begin\{rSection\}\{(?:EXPERIENCE|WORK EXPERIENCE|PROFESSIONAL EXPERIENCE)\}(.*?)\\end\{rSection\}/s,
          /\\section\*?\{(?:Experience|Work Experience|Professional Experience)\}(.*?)(?=\\section|\s*$)/s,
        ],
      },
      {
        name: "Education",
        patterns: [
          /\\begin\{rSection\}\{EDUCATION\}(.*?)\\end\{rSection\}/s,
          /\\section\*?\{Education\}(.*?)(?=\\section|\s*$)/s,
        ],
      },
      {
        name: "Skills",
        patterns: [
          /\\begin\{rSection\}\{(?:SKILLS|TECHNICAL SKILLS|CORE COMPETENCIES)\}(.*?)\\end\{rSection\}/s,
          /\\section\*?\{(?:Skills|Technical Skills|Core Competencies)\}(.*?)(?=\\section|\s*$)/s,
        ],
      },
      {
        name: "Projects",
        patterns: [
          /\\begin\{rSection\}\{PROJECTS\}(.*?)\\end\{rSection\}/s,
          /\\section\*?\{Projects\}(.*?)(?=\\section|\s*$)/s,
        ],
      },
    ];
    ...
    sectionPatterns.forEach(({ name, patterns }) => {
      for (const pattern of patterns) {
        const match = latexCode.match(pattern);
        if (match) {
          const content = this.cleanLatexContent(match[1]);
          if (content.trim()) {
            data.sections.push({
              title: name,
              content: content,
    ...
    // If no name found, try to extract from first large text
    if (!data.name) {
      const fallbackName = latexCode.match(/\\textbf\{([^}]{3,50})\}/);
      if (fallbackName) {
        data.name = fallbackName[1].trim();
      }
    }
  ...
  getServiceInfo() {
    return {
      name: "Enhanced LaTeX-PDFKit Generator",
      version: "1.1.0",
      description:
        "High-quality PDF generation using enhanced manual LaTeX parsing and professional PDFKit rendering",
      features: [
        "Enhanced manual LaTeX parsing",
        "Professional PDF rendering with PDFKit",
        "Intelligent section detection",
        "High-quality typography",
        "Overleaf-style output",
        "Robust contact information extraction",
        "Multiple LaTeX format support",
      ],
    };
  }
}
