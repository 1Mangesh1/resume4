  ...
  async generateBestResume(resumeText, analysisData = null) {
    try {
      ...
      const promptBuilder = new PromptBuilder()
        ...
        .setToneContext(
          "Concise, modern, and results-focused. Resume must use impactful bullet points, strong action verbs, quantifiable results, and be scannable by both humans and ATS."
        )
        .setBackgroundData(backgroundData)
        .setTaskDescription(
          "Rewrite, optimize, and format the resume into a visually appealing, professional LaTeX file with excellent spacing, alignment, and strategic bolding:",
          [
            ...
            "Use proper page margins with \\geometry{left=0.75in,right=0.75in,top=0.75in,bottom=0.75in}.",
            ...
            "Header (Name): Use {\\LARGE \\textbf{FULL NAME}} for prominent name display.",
            "Contact Info: Place email, phone, LinkedIn, GitHub on same line, separated by $\\cdot$ with proper spacing.",
            "Section Headers: Use \\section*{\\Large Section Name} for clear visual hierarchy.",
            "Experience/Education Layout: CRITICAL - Use this exact format for each entry:",
            "  \\textbf{Job Title/Degree} \\hfill Time Period \\\\",
            "  Company/Institution — Location",
            "  This creates perfect left-right alignment with job title/degree left-aligned and dates right-aligned.",
            "Spacing: Add \\vspace{2pt} between major entries for visual breathing room.",

            // --- STRATEGIC BOLDING RULES ---
            "Bold ONLY these elements for maximum impact:",
            "  - Full name in header",
            "  - Job titles/positions in experience section",
            "  - Degree names in education section",
            "  - Project names in projects section",
            "  - Category labels in skills (e.g., \\textbf{Programming Languages:})",
            "DO NOT bold: company names, universities, locations, dates, bullet point content, or section headers.",

            // --- BULLET REWRITE RULES ---
            "Rewrite all experience and project bullet points to:",
            "  - Start with a strong action verb (e.g., Designed, Developed, Optimized, Led, Automated, Improved, Deployed, Implemented).",
            "  - Include quantifiable results where possible (e.g., %, $, time saved, performance improvement, revenue impact).",
            "  - State the business or technical impact of the action.",
            ...
            "Order sections as: Header, Education, Skills, Experience, Projects.",
            "Skills section: Group into logical categories with bold labels (e.g., \\textbf{Programming Languages:} Python, JavaScript, TypeScript).",
            "Use itemize environment with [leftmargin=15pt] for consistent bullet indentation.",
            "Ensure clean line breaks with \\\\ and proper spacing throughout.",
            "Make the resume scannable by both humans and ATS systems.",
            ...
            "Ensure consistent grammar, spelling, and tense throughout.",
          ]
        )
        .setImmediateTask(
          "Generate a visually stunning, professionally formatted LaTeX resume with perfect alignment, strategic bolding, and optimal spacing. Emphasize: left-aligned job titles with right-aligned dates using \\hfill, strategic bolding for key elements only, proper visual hierarchy with \\Large sections, and compelling action-oriented bullet points. The result should look clean, modern, and highly readable both on screen and in print."
        )
        .setOutputFormat(
          "Return only the complete LaTeX code, starting with \\documentclass and ending with \\end{document}. Focus on visual appeal, professional formatting, and flawless compilation."
        );
