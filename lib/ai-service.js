...
class AIService {
  ...
  isJobTitle(line) {
    const jobTitles = [
      ...
      'lead', 'senior', 'junior', 'intern', 'trainee'
    ];
  ...
  extractEducation(resumeText) {
    ...
    const educationKeywords = ['education', 'bachelor', 'master', 'phd', 'university', 'college', 'institute', 'degree'];
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (educationKeywords.some(keyword => lineLower.includes(keyword))) {
        ...
        for (let j = i; j < Math.min(i + 4, lines.length); j++) {
          const degreeMatch = degreePatterns.find(pattern => pattern.test(lines[j]));
          if (degreeMatch) {
            const degreeLine = lines[j];
            const yearMatch = lines.slice(j, j + 3).join(' ').match(/20\d{2}/g);
            ...
            for (let k = Math.max(0, j - 2); k < Math.min(j + 3, lines.length); k++) {
              if (k !== j && (lines[k].toLowerCase().includes('university') || 
            ...
            educationSections.push({
              ...
              gpa: this.extractGPA(lines.slice(Math.max(0, j - 2), j + 3).join(' '))
            });
   ...
   */
  extractGPA(text) {
    const gpaMatch = text.match(/gpa[:\s]*(\d+\.?\d*)/i) || text.match(/(\d+\.?\d*)\s*\/\s*(\d+)/);
    if (gpaMatch) {
      return gpaMatch[1];
    }
    return null;
  ...
  extractExperience(resumeText) {
    ...
    // Company patterns
    const companyIndicators = ['llc', 'inc', 'corp', 'ltd', 'pvt', 'technologies', 'solutions', 'systems', 'software'];
      ...
      if (jobTitlePatterns.some(pattern => pattern.test(line))) {
        ...
        for (let j = Math.max(0, i - 2); j < Math.min(i + 3, lines.length); j++) {
          if (j !== i) {
            ...
            if (companyIndicators.some(indicator => checkLine.includes(indicator)) ||
        ...
        for (let j = i + 1; j < Math.min(i + 6, lines.length); j++) {
          ...
          } else if (jobTitlePatterns.some(pattern => pattern.test(achievementLine))) {
  ...
  extractProjects(resumeText) {
    ...
    // Project indicators
    const projectKeywords = ['project', 'portfolio', 'github', 'application', 'system', 'app', 'website', 'platform'];
    const techKeywords = ['javascript', 'python', 'react', 'node', 'django', 'angular', 'vue', 'html', 'css'];
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inProjectsSection || projectKeywords.some(keyword => lineLower.includes(keyword))) {
        // Skip lines that are too long or look like descriptions
        if (line.length > 100 || line.includes('•') || line.includes('developed')) {
          continue;
        }
        ...
        for (let j = i + 1; j < Math.min(i + 4, lines.length); j++) {
          ...
          if (descLine.length > 20 && !techKeywords.some(tech => descLine.toLowerCase().includes(tech))) {
  ...
  extractSkills(resumeText, aiAnalysis) {
    ...
    const toolSkills = [
      ...
      'aws', 'azure', 'gcp', 'firebase', 'vercel', 'heroku',
      'mysql', 'postgresql', 'mongodb', 'redis', 'elasticsearch'
    ];
    
    const languageSkills = ['english', 'spanish', 'french', 'german', 'chinese', 'japanese', 'hindi', 'marathi'];
  ...
  async generateBestResume(resumeText, analysisData = null) {
    try {
      ...
      const promptBuilder = new PromptBuilder()
        .setTaskContext(
          "You are an expert LaTeX resume designer and career coach with deep knowledge of professional resume formatting, ATS optimization, and modern design principles."
        )
        .setToneContext("Professional, clean, and optimized for maximum impact")
        .setBackgroundData(backgroundData)
        .setTaskDescription(
          "Generate a complete, professional LaTeX resume that is ATS-friendly, visually appealing, and highlights the candidate's strengths effectively.",
          [
            "Use modern LaTeX packages for professional formatting",
            "Include proper document structure with clear sections",
            "Optimize for ATS scanning with clean, semantic markup",
            "Do NOT make the layout overly compact — use comfortable whitespace and spacing",
            "Increase readability with generous margins (~0.75in–1in), balanced line spacing (e.g., setspace or \parskip), and space between sections",
            "Use compact bullet lists but keep at least small vertical space between items for readability",
            "Avoid cramming content; allow breathing room around headings and entries",
            "Highlight key achievements and quantifiable results",
            "Ensure print compatibility",
            "Include proper contact information formatting",
            "Maintain consistent styling throughout"
          ]
        )
        .setConversationHistory([])
        .setImmediateTask(
          "Create a complete LaTeX document that compiles into a professional resume ready for job applications"
        )
        .setOutputFormat(
          "Return ONLY the complete LaTeX code starting with \\documentclass and ending with \\end{document}. Do not include any explanations, markdown formatting, or additional text."
        );
  ...
  // Clean up the response to ensure it's pure LaTeX
      ...
      try {
        const usesHtmlDefineColor = /\\definecolor\{[^}]+\}\{HTML\}\{[0-9A-Fa-f]{6}\}/.test(latexCode);
        ...
        if (usesHtmlDefineColor && !hasXcolor) {
          ...
          latexCode = latexCode.replace(/(\\documentclass[^\n]*\n)/, `$1\\usepackage{xcolor}\n`);
        }
      } catch (_) {}
