/**
 * AI Service Integration with structured prompts and AI SDK
 * Handles all AI interactions with proper error handling and security
 */
...
class AIService {
  ...
  /**
   * Analyze resume with structured prompts and output
   */
  async analyzeResume(
    resumeText,
    jobDescription = null,
    ...
    try {
      ...
      // Build structured prompt with conditional JD handling
      ...
      if (generatorOptions.includeSummaryGen) {
        generatorSections += `\n- resume_summary: Generate a concise, impactful professional summary (2-4 sentences) tailored to the resume (and job description if provided). Include keyword_density (number) and improvement_suggestions (array).`;
      }
      if (generatorOptions.includeVariantGen) {
        generatorSections += `\n- tailored_resume: Generate a tailored summary, enhanced bullet points, and skills optimization. If job description is provided, maximize alignment. Include enhanced_bullets (array), skills_optimization (object), ats_keywords (array, if JD), match_percentage (number, if JD).`;
      }
      if (generatorOptions.includeCoverGen) {
        generatorSections += `\n- cover_letter: Generate a personalized cover letter. Structure as JSON with greeting, intro, body, closing, signature, full_letter, key_strengths_highlighted, word_count, tone_analysis, and role_specific_content (boolean).`;
      }
      if (generatorOptions.includeLinkedInGen) {
        generatorSections += `\n- linkedin_summary: Generate a LinkedIn About section. Include keyword_density, structure_analysis, optimization_score, readability_tips.`;
      }
      if (generatorOptions.includeLatexGen) {
        generatorSections += `\n- latex_resume: Generate a LaTeX resume source. Include latex_source (string) and template_used (string).`;
      }

      // Replace placeholder in outputFormat
  ...
  async generateCoverLetter(
    ...
    try {
      ...
      // Build background data conditionally
  ...
  async streamAnalysis(resumeText, jobDescription = null) {
    try {
      ...
      // Build structured prompt with conditional JD handling for streaming
      const hasJobDescription = !!jobDescription;
